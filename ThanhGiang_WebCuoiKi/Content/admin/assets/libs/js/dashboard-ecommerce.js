...
// Biến để lưu trữ số dòng có giới tính là "nam"
var countMale = 0;
var countFMale = 0;
var countUnknown = 0;
// Duyệt qua từng dòng và kiểm tra giới tính
rows.forEach(function (row) {
    // Lấy ô chứa giới tính trong mỗi dòng
    var genderCell = row.querySelector("td:nth-child(5)");

    //Kiểm tra nếu giới tính là "nam" thì tăng biến đếm
    if (genderCell.textContent.trim().toLowerCase() == "nam") {
        countMale++;
    }
    else if (genderCell.textContent.trim().toLowerCase() == "nu") {
        countFMale++;
    } else {
        countUnknown++;
    }
    ...
    var chart = new Chartist.Pie('.ct-chart-category', {
        series: [countMale, countFMale,countUnknown] 
